...
public class CopyOfUrlFormatUtil_bak {

	private static final Logger logger = Logger.getLogger(CopyOfUrlFormatUtil_bak.class);
	...
	public static String formatUrl(String param) {
		...
		String qy_api_url = (String) Configuration.getProperty("qy_api_url");
		String qy_api_uri = (String) Configuration.getProperty("qy_api_uri");

		// 根据API地址、param拼装请求URL
		if (StrUtil.isNullStr(qy_access_key_id)) {
			return "请在配置文件config.properties中设置API密钥IDaccess_key_id";
		}

		if (StrUtil.isNullStr(qy_secret_access_key)) {
			return "请在配置文件config.properties中设置API密钥的私钥qy_secret_access_key";
		}

		if (StrUtil.isNullStr(qy_api_url)) {
			return "请在配置文件config.properties中设置青云API地址qy_api_url，默认为 https://api.qingcloud.com/iaas/";
		}

		if (StrUtil.isNullStr(qy_api_uri)) {
			return "请在配置文件config.properties中设置青云请求的uri路径，默认为 /iaas/";
		}
		...
		try {
			for (String key : resultMap.keySet()) {
				if (i == 0) {
					...
					strSignature.append("&" + getUrlEncoder(key) + "=" + getUrlEncoder(map.get(key).toString()));
					strApiUrl.append("&" + getUrlEncoder(key) + "=" + getUrlEncoder(map.get(key).toString()));
				}
				i++;
			}
